...
public class JobApplicationResource {
	...
	public ResponseEntity<CommonApiResponse> updateJobApplicationStatus(JobApplication request) {
		...
		if (request.getStatus().equals(JobApplicationStatus.SHORTLISTED.value())) {
			String mailBody = sendEmployeeShortlistedMail(jobApplication.getEmployee(), jobApplication);
			String subject = "Congratulations! You've Been Shortlisted - Online Job Portal";

			this.emailService.sendEmail(jobApplication.getEmployee().getEmailId(), subject, mailBody);
		}
	...
	private String sendEmployeeShortlistedMail(User customer, JobApplication jobApplication) {
	    ...
	    emailBody.append("<p>Congratulations! You have been shortlisted for the position you applied for. Your Application ID is <b>" + jobApplication.getApplicationId() + "</b>.</p>");
	    
	    // Company details
	    emailBody.append("<h3>Company Details:</h3>");
	    emailBody.append("<p><b>Company Name:</b> " + jobApplication.getJob().getCompanyName() + "</p>");
	    emailBody.append("<p><b>Job Title:</b> " + jobApplication.getJob().getTitle() + "</p>");
	    emailBody.append("<p><b>Job Type:</b> " + jobApplication.getJob().getJobType() + "</p>");
	    emailBody.append("<p><b>Employer Name:</b> " + jobApplication.getJob().getEmployer().getFirstName() + " " + jobApplication.getJob().getEmployer().getLastName() + "</p>");
	    
	    // Closing
	    emailBody.append("<p>Best Regards,<br/>Online Job Portal</p>");
